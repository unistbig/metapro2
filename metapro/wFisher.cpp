...
void wFisher(vector<string> id, double ** pmat, double ** dmat, int tt, vector<int> SS, int nFeat, int nP, string output_prefix)
{
	...
	ofstream OUT;
	string output = output_prefix + "_wFisher.out";
	...
	for(i = 0; i < nFeat; i++)
	{
		...
		if(tt == 2)
		{
			...
			Pval_pos = gsl_cdf_gamma_Q(Gsum_pos, nP, 2);
			Pval_neg = gsl_cdf_gamma_Q(Gsum_neg, nP, 2);
			...
			finalP = gsl_cdf_gamma_Q(Gsum, nP, 2);
		}
		// write result
		OUT << id[i] << "\t" << finalP << "\n";
	}
